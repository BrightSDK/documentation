...
<Steps>
  ...
  <Step title="Extract SDK Framework" titleSize="h3">
    ...
    **SDK Overview:**
    ...
    - **SDK Integration Components:**
      ...
      - **brd\_api.js** - Frontend interface implementing the consent screen. Must be integrated with your application's frontend.
  </Step>
  <Step title="Integrate Bright SDK in Your Code" titleSize="h3">
    1. **Prepare the Files**
       ...
       <Note>
         **Package ID Naming Tip:** After creating a Tizen app, we recommend changing the package ID to something more readable using your partner ID from the Control Panel. Examples: `blUqHYQcnKEE.actionGames` → `brightdata.actionGames` `super_games` → `SuperGames` (remove illegal characters)
       </Note>
       <Info>
         The `tizen_ prefix` will be added automatically by the SDK.
       </Info>
    2. **Embed SDK Dialog in Your Code** You can use `sample_app` as a reference. **Import brd\_api.js**
