...
import IntegrationMethods from "/snippets/win-integration-methods.mdx";
...
<Steps>
  ...
  <Step title="Install SDK Files" titleSize="h3">
    ...
    - **Manually** - download the SDK from the dashboard and copy the files yourself.
    ...
    <AccordionGroup>
      <Accordion title="Option A - Using the Integration Tool (Recommended)">
        Run the tool using the [bright-sdk-integration](https://brightsdk.github.io/bright-sdk-integration/) CLI. See the [Windows example](https://github.com/BrightSDK/bright-sdk-integration/blob/main/example/windows/README.md) for full setup instructions.
      ...
      <Accordion title="Option B - Manual">
        Download the latest Bright SDK from the [Bright SDK Dashboard](https://bright-sdk.com/cp/docs/sdk/windows) and unzip it.
  ...
  <Step title="Integrate Bright SDK in Your Code" titleSize="h3">
    ...
    **Add SDK Initialization to Your Code**
    ...
    4. Write handler code for the user choice changed event.
       - _For example:_ enable/disable ads or premium features.
  ...
  <Step title="Add SDK Service Status Notifications (Optional)" titleSize="h3">
    ...
    **Implementation:** Add delegate for the `BrightData.Api.ServiceStatusChanged` event, the handler for the event is an EventHandler that receives the `BrightData.Api.ServiceStatusChangedEventArgs` as event data.
  ...
  <Step title="API Documentation" titleSize="h3">
    ...
    <AccordionGroup>
      ...
      <Accordion title=".NET API Reference">
        **Methods**
      ...
      <Accordion title="Settings Class">
      ...
      <Accordion title="Command Line Tool (net_updater32.exe)">
        **Help & Information**
