...
1. **Adding Opt-In/Out to Your Existing Settings Menu** **Requirements:**
   1. <u>Add a switch labeled "Web Indexing"</u>
   ...
   2. <u>Add value text below the switch</u>
      Emphasize the benefits users get when opting in. ([see examples](#:~:text=Value%20Text%20Examples%3A))
   ...
   5. Behavior
      ...
      - Turning ON → call `brd_api.show_consent()` (user action should open consent). **Examples of opted-in and opted-out scenarios:**
   <Columns cols={2}>
...
2. **If You Don't Have a Settings Menu** You can use the ready-to-use simple opt-out UI.
   1. **How It Works:**
...
3. **Consent Dialog Customization (Optional)** You can customize the consent dialog by setting additional parameters in the `api_settings` object passed to `init()`. Customizable fields:
   <img
